...
shinyUI(
        fluidPage(
        titlePanel("Healthy Detector for Adults"),
        sidebarPanel(
                ...
                selectInput(inputId="country",label="Country",choices=sort(ISO_3166_1$Alpha_3),multiple=FALSE, selected="USA"),
                conditionalPanel(
                        condition="input.country == 'USA'",
                        p("Please choose a state"),
                        selectInput(inputId="state", label="State",choices=state.name,multiple=FALSE,selected=NULL)),
